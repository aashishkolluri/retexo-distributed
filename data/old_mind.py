...
class DistrMindDataset(DistrRelBenchDataset):
    name = "mind"

    # training on 5th week, validation last day of 5th week, testing on 6th week.
    val_timestamp = pd.Timestamp("2019-11-01")  # TODO
    test_timestamp = pd.Timestamp("2019-11-02")  # TODO
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(partition_dir=partition_dir,
                         part_id=part_id, distributed=distributed)
    ...
    def make_db(self) -> Database:
        ...
        urls = {
            ...
            "val": {
                    ...
                    "hash": "17d4951f026aaafba4dabc0ea723da936df5609288649e000e381c90fbec3bf0"
                },
            "test": {
                    "url": "https://mind201910small.blob.core.windows.net/release/MINDlarge_test.zip",
                    "hash": "9dd07b311a8f02ff5a9c3bc616dcf0d38821efcdd9f1e494cfb70062e58106cd"
                }
        } 
        ...
        for batch, item in urls.items():
            url = item["url"]
            path = pooch.retrieve(
                url,
                known_hash=item["hash"],
                progressbar=True,
        ...
        postHistory["CreationDate"] = pd.to_datetime(
    ...
    def shardDataset(self, num_shards: int = 2, folder: Union[str, os.PathLike] = "data") -> List[Database]:
        ...
        remaining_votes = self.retrieve_foreign_rows(
            shards[0]["df"], vote_shards, remaining_votes, votes.df, foreignKey="UserId")

        votes.df = self.update_part_id(votes.df, vote_shards)

        # Filling the shards with the remaining votes
        vote_shards = self.fill_shards(
            num_shards, votes, vote_shards, remaining_votes)
        shards.append(
            {"name": "votes",
             "df": vote_shards,
        ...
        remaining_posts = self.retrieve_foreign_rows(
            shards[0]["df"], post_shards, remaining_posts, posts.df, foreignKey="OwnerUserId")

        posts.df = self.update_part_id(posts.df, post_shards)

        remaining_posts = self.retrieve_foreign_rows(
            shards[1]["df"], post_shards, remaining_posts, posts.df, OnKey="PostId", foreignKey="Id")

        post_shards = self.fill_shards(
            num_shards, posts, post_shards, remaining_posts)
        shards.append(
            {"name": "posts",
             "df": post_shards,
        ...
        remaining_badges = self.retrieve_foreign_rows(
            shards[0]["df"], badge_shards, remaining_badges, badges.df, foreignKey="UserId")
        badges.df = self.update_part_id(badges.df, badge_shards)

        badge_shards = self.fill_shards(
            num_shards, badges, badge_shards, remaining_badges)
        shards.append(
            {"name": "badges",
             "df": badge_shards,
        ...
        remaining_comments = self.retrieve_foreign_rows(
            shards[0]["df"], comment_shards, remaining_comments, comments.df, foreignKey="UserId")
        comments.df = self.update_part_id(comments.df, comment_shards)
        remaining_comments = self.retrieve_foreign_rows(
            shards[2]["df"], comment_shards, remaining_comments, comments.df, foreignKey="PostId")
        comments.df = self.update_part_id(comments.df, comment_shards)

        comment_shards = self.fill_shards(
            num_shards, comments, comment_shards, remaining_comments)
        shards.append(
            {"name": "comments",
             "df": comment_shards,
        ...
        remaining_postHistory = self.retrieve_foreign_rows(
            shards[0]["df"], postHistory_shards, remaining_postHistory, postHistory.df, foreignKey="UserId")
        postHistory.df = self.update_part_id(
            postHistory.df, postHistory_shards)
        remaining_postHistory = self.retrieve_foreign_rows(
            shards[2]["df"], postHistory_shards, remaining_postHistory, postHistory.df, foreignKey="PostId")
        postHistory.df = self.update_part_id(
            postHistory.df, postHistory_shards)

        postHistory_shards = self.fill_shards(
            num_shards, postHistory, postHistory_shards, remaining_postHistory)
        shards.append(
            {"name": "postHistory",
        ...
        remaining_postLinks = self.retrieve_foreign_rows(
            shards[2]["df"], postLinks_shards, remaining_postLinks, postLinks.df, foreignKey="PostId")
        postLinks.df = self.update_part_id(postLinks.df, postLinks_shards)
        remaining_postLinks = self.retrieve_foreign_rows(
            shards[2]["df"], postLinks_shards, remaining_postLinks, postLinks.df, foreignKey="RelatedPostId")
        postLinks.df = self.update_part_id(postLinks.df, postLinks_shards)

        postLinks_shards = self.fill_shards(
            num_shards, postLinks, postLinks_shards, remaining_postLinks)
        shards.append(
            {"name": "postLinks",
             "df": postLinks_shards,
        ...
        for shard in shards:
            ...
            for i in range(num_shards):
                print("Shard " + str(i) + " is " +
                      str(shard["df"][i].shape[0]))
                sum += shard["df"][i].shape[0]

            duplicated = sum - total
            print("Portion of data duplicated " +
                  str((duplicated/total*100)) + "%")
        ...
        for shard in shards:
            ...
            for i in range(num_shards):
                ...
                assert shard["df"][i].nunique(
                )["Id"] == shard["df"][i].shape[0]

            uniques = df.drop_duplicates(
                subset=['Id', 'part_id'], keep='first')
            count_partitions_per_id = uniques.groupby(
                'Id')['part_id'].nunique()
        ...
        for i in range(num_shards):
            ...
            torch.save(local_to_global_id_dict,
                       f"{folder}/shard_{i}/local_to_global_id_dict.pt")
