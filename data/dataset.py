...
def rel_graph_partition(
    ...
    if dataset_name == "rel-stackex":
        ...
        dataset = DistrMindDataset(partition_dir=partition_dir)
    else:
        raise NameError("no implementation for given dataset")
    
    if not os.path.exists(partition_dir):
        os.makedirs(partition_dir)
    if not os.path.exists(os.path.join(cfg.partition_dir, f"materialized_cache")):
        os.makedirs(os.path.join(cfg.partition_dir, f"materialized_cache")) # clear cache
